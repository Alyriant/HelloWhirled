...
/**
 * A simple trie for Strings, supporting add, containsPrefix, contains, and remove. To support
 * Unicode efficiently, strings are first converted to UTF-8 internally, and then a traditional byte
 * array approach is used for storage. Keys are converted to lowercase, and the string is converted
 * to Unicode Normalization Form KC (NFKC): Compatibility Decomposition, followed by Canonical
 * Composition. This converts, for example, the ligature ﬀ into the characters ff, so "Oﬀ" and "Off"
 * will be the same key, "off".
 *
...
public class SimpleTrie {
    ...
    /**
     * Converts a non-negative integer (0 to 255) to a signed byte -128 to 127. Useful when
     * converting an array index to a byte value.
     */
    ...
    /**
     * Converts a byte (which is a signed number from -128 to 127) to the equivalent non-negative
     * integer (0 to 255). Useful when using the byte value as an array index.
     */
    ...
    /**
     * Adds a string to the trie, if it doesn't already exist. Keys are converted to lowercase.
     * Unicode is supported, with a normalized form to avoid inadvertent duplicate keys.
     */
    ...
    /**
     * Helper for the public printKeys(). Prints the current key if this node has one, then prints
     * the keys of any words that this is a prefix of.
     *
